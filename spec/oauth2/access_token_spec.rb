...
describe AccessToken do
  ...
  describe '#expired?' do
    ...
    context 'when time stubbed' do
      let!(:access) { AccessToken.new(client, token, refresh_token: 'abaca', expires_in: 600) }

      before do
        allow(Time).to receive(:now).and_return(Time.now + 10800)
      end
